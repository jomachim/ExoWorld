class Entity {
	...
	var interpolateSprPos = true;//false;
	...
	**/
	...
	public function say(str:String, c=0xffffff) {
		...
		//var s = Assets.tiles.h_get( D.tiles.SayLine, saying );
		var s = Assets.tiles.h_get( D.tiles.SayLine, saying );
		s.colorize(c);
		saying.addChild(tf);
		//game.scroller.add(saying, Const.DP_UI);
		var mod=new ui.Window();
		mod.add(saying);
	}
